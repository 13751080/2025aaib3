...
public:
    int calPoints(vector<string>& operations) {
        ...
        for ( string op : operations) { // Part01: C++ 進階迴圈
            ...
            } else if (op[0]=='D') { // 最後一位「變2倍」再「加到最後面」
                a.push_back( a.back() * 2); // 成兩倍 Part06: back()
            } else if (op[0]=='+') { // 還不知道, 等一下再看
                int temp = a.back(); // 先抄後1台車的車牌
                a.pop_back(); // 把最後一台車開走
        ...
        for (int now : a) { // Part05: C++ 進階迴圈 要看陣列裡的值, 把全部加起來
            ans += now; // cout << now << " "; // Part02: 看它是誰, 等一下刪掉 (看陣列裡的值)
        }
        return ans; // return 0; // 先隨便 return 等一下再塞「真的答案」
    }
};
